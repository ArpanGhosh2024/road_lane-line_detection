...
def region_of_interest(image):
    ...
    ])  #the coordinates of the triangle(area of interest)
...
while(cap.isOpened()):
    ...
    cv2.imshow("Result",combo_image)
    if cv2.waitKey(1)==ord("q"): #waitkey(1)waits 1ms between each frame. It also returns a 32 bit integer value. This property can be used to close the video on pressing a key(here 'q')
